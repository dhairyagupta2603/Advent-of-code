...
template <size_t N>
int64_t check_blocking_byte(std::array<std::array<cell_t, N>, N> grid) {
    search(grid, index_t{0, 0}, 0);
    return grid[N - 1][N - 1].cost;
}

index_t solve(std::vector<index_t> indicies) {
    const size_t dim = 71;
    const ssize_t num_bytes = static_cast<ssize_t>(indicies.size());
    ...
    for (ssize_t i = 0; i < num_bytes; i++) {
        const auto idx = index_t{indicies[i].i, indicies[i].j};
        grid[idx.i][idx.j].ch = '#';

        if (check_blocking_byte(grid) == std::numeric_limits<int64_t>::max()) {
            return idx;
        }
    }
    return index_t{};
