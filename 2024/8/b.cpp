...
std::vector<Anti_Node> get_anti_nodes(const Matrix<Index>& grid, const Index& a, const Index& b) {
    const auto get_harmonics = [&grid, &a, &b](const Index& antenna, const std::complex<long>& vec,
                                               std::vector<Anti_Node>& anti_nodes) {
        long n = 0;
        std::complex<long> an;
        while (true) {
            an = antenna.pos + n * vec;
            if (!valid_index(grid, an.real(), an.imag())) break;
            anti_nodes.emplace_back(Anti_Node{a, b, Index{an}});
            n++;
        }
    };
    ...
    get_harmonics(b, complex_vec, anti_nodes);
    get_harmonics(a, -complex_vec, anti_nodes);
